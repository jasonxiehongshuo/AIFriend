...
<template>
  <div class="dropdown dropdown-end">
    ...
    <ul tabindex="-1" class="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm">
      ...
      <li>
        <RouterLink @click="closeMenu" :to="{name: 'user-space-index',params:{user_id: user.id}}" class="text-sm front-bold py-3">
          <UserSpaceIndex />
          个人空间
        </RouterLink>
      </li>
      <li>
        <RouterLink @click="closeMenu" :to="{name: 'user-profile-index'}" class="text-sm front-bold py-3">
          <UserProfileIcon />
          编辑资料
        </RouterLink>
      </li>
      <li/>
      <li>
        <a @click="closeMenu" class="text-sm front-bold py-3">
          <UserLogoutIcon />
          退出登录
        </a>
      </li>
